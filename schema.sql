...
CREATE TABLE IF NOT EXISTS `medicare`.`medicare_services_test_result_name` (
  `mt_id` INT NOT NULL AUTO_INCREMENT,
  `mt_ms_id` INT NULL,
  `mt_tn_id` INT NULL,
  PRIMARY KEY (`mt_id`),
  INDEX `mt_ms_fk_idx` (`mt_ms_id` ASC),
  INDEX `mt_tn_fk_idx` (`mt_tn_id` ASC),
  CONSTRAINT `mt_ms_fk`
    FOREIGN KEY (`mt_ms_id`)
    REFERENCES `medicare`.`medicare_services` (`ms_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `mt_tn_fk`
    FOREIGN KEY (`mt_tn_id`)
    REFERENCES `medicare`.`test_result_name` (`tn_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
